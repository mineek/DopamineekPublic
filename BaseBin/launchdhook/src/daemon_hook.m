...
xpc_object_t xpc_dictionary_get_value_hook(xpc_object_t xdict, const char *key)
{
	...
	if (!strcmp(key, "LaunchDaemons")) {
		if (xpc_get_type(origXvalue) == XPC_TYPE_DICTIONARY) {
			for (NSString *daemonPlistName in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:@"/basebin/LaunchDaemons" error:nil]) {
				if ([daemonPlistName.pathExtension isEqualToString:@"plist"]) {
					xpc_dictionary_add_launch_daemon_plist_at_path(origXvalue, [@"/basebin/LaunchDaemons" stringByAppendingPathComponent:daemonPlistName].fileSystemRepresentation);
				}
			}
		...
		if (xpc_get_type(origXvalue) == XPC_TYPE_ARRAY) {
			xpc_array_set_string(origXvalue, XPC_ARRAY_APPEND, "/basebin/LaunchDaemons");
